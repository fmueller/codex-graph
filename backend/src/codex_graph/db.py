...
def _get_git_repo_root(start_dir: Path) -> Path | None:
...
def _get_git_commit_info(file_path: str) -> GitCommitInfo | None:
    resolved = Path(file_path).resolve()
    repo_root = _get_git_repo_root(resolved.parent)
...
async def _persist_file_ast_to_age(engine: AsyncEngine, fa: FileAst, file_path: str) -> None:
    ...
    git_info = _get_git_commit_info(file_path)
    ...
    cypher_fv = (
        ...
        f"fv.author = '{_escape_str(author)}' "
        f"RETURN id(fv)"
    )
