...
def main() -> None:
    ...
    async def _runner() -> None:
        engine = _get_engine()
        try:
            file_uuid = await _persist_file(engine, str(file_path))
            print(f"Persisted file {file_path} with UUID {file_uuid}")

            ast = _extract_ast_from_file(str(file_path), file_uuid, resolved_language)
            print(f"Extracted AST from {file_path}")

            await _persist_file_ast_to_age(engine, ast, str(file_path))
